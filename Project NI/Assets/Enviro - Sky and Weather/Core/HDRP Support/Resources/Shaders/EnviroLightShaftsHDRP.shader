...
Shader "Enviro/Pro/LightShafts" {
	...
	//	SAMPLE_TEXTURE2D_X(_ColorBuffer, s_linear_clamp_sampler, positionSS);
	...
	half4 frag_depth (Varyings input) : SV_Target {
		...
		float depthSample = Linear01Depth(LOAD_TEXTURE2D_X(_CameraDepthTexture, positionSS).r, _ZBufferParams);
		//float depthSample = LoadCameraDepth(positionSS);
		//depthSample = Linear01Depth(depthSample, _ZBufferParams);

		half4 tex = LOAD_TEXTURE2D_X(_MainTex, positionSS);
		

		// consider maximum radius
		//half2 vec = ClampAndScaleUVForBilinear(input.blurVector);
	...
	half4 frag_nodepth (Varyings input) : SV_Target {
		...
		float4 tex = LOAD_TEXTURE2D_X(_MainTex, positionSS);
