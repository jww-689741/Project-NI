    ...
    SubShader{
        ...
        HS_Input VS(uint ID : SV_VERTEXID)
        {
          ...
          return Output;
            ...
            float4 PS(PS_Input IN, out float4 g1 : SV_TARGET1, out float4 g2: SV_TARGET2, out float4 g3 : SV_TARGET3) : SV_TARGET
            {
              float3 N = GetNormal(IN.uv, 1.0f / (Resolution - 1));
              float light = saturate(dot(N, length(_WorldSpaceLightPos0) > 0.5f ? _WorldSpaceLightPos0 : float3(0,1,0)));
              g1 = float4(0,0,0,0);
              g2 = float4(1,0,1,1);
              g3 = float4(1, 1, 1, 1);
              return float4(light.xxx + 0.15f,1);
            }
