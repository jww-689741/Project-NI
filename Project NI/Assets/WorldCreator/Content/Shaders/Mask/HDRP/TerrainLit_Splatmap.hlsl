...
#undef SampleResults
    ...
    float2 pixPos = (controlUV - brushMin) / _BrushSize.xx;
    ...
    float brushValue = SAMPLE_TEXTURE2D(_Brush, sampler_Brush, pixPos).r * _BrushStrength * brushBorder;
    float maskValue = SAMPLE_TEXTURE2D(_Mask, sampler_Mask, controlUV).r;
      
    float finalValue = saturate(maskValue + brushValue);
    surfaceData.albedo = lerp(float3(1,1,1), float3(1,0,0), finalValue);    
