        ...
        HS_Input VS(uint ID : SV_VERTEXID)
        {
          ...
          return Output;
            ...
            float4 PS(PS_Input IN) : SV_TARGET
            {
              ...
              float2 pixPos = (IN.uv - brushMin) / _BrushSize.xx;
              ...
              float maskValue = tex2D(_Mask, IN.uv).r;

              float3 finalValue = lerp(1, float3(1,0,0), saturate(maskValue + brushValue));

              return float4(finalValue * (light + 0.15f),1);
            }
