...
public class PlayerControl : MonoBehaviour
{
    ...
    private void Start()
    {
       ...
       // Turret = ObjectManager.instance.GetBullet("Test3");
    }
    ...
    private void Shot()
    {
        GameObject Turret = ObjectManager.instance.GetBullet("Test3");
        ...
        if (Input.GetMouseButtonDown(3)) // 마우스 추가버튼 유지중일때
        {
            duration = Time.time;
            // GameObject Turret = ObjectManager.instance.GetBullet("Test3");
            Turret.transform.position = new Vector3(this.transform.position.x, this.transform.position.y, (this.transform.position.z + 5f)); // 위치 지정
            Turret.SetActive(true);
        }else if (Input.GetMouseButtonUp(3)) repeaterLock = false; // 마우스 추가버튼에서 손을 뗄때
