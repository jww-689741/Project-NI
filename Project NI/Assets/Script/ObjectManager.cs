...
public class ObjectManager : MonoBehaviour
{
    ...
    public List<Queue<GameObject>> enemyList; // 적 오브젝트의 리스트
    public List<Queue<GameObject>> bulletList; // 탄환 오브젝트의 리스트
    ...
    Queue<GameObject> EnqueueObject(EnemyInfo element) // 리스트 내부 큐에 정보가 등록된 오브젝트의 클론을 생성하고 저장하는 메소드 - 적 오브젝트 전용
    {
        Queue<GameObject> returnQueue = new Queue<GameObject>(); // 반환 할 큐

        for(int i = 0; i < element.count; i++) // 등록한 카운트만큼 루프
        {
            GameObject clone = Instantiate(element.prefab) as GameObject; // 등록한 오브젝트의 클론 생성
    ...
    Queue<GameObject> EnqueueObject(BulletInfo element) // 리스트 내부 큐에 정보가 등록된 오브젝트의 클론을 생성하고 저장하는 메소드 - 탄환 오브젝트 전용
    {
        Queue<GameObject> returnQueue = new Queue<GameObject>(); // 반환 할 큐
